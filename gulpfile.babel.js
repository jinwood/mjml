...
const compile = () => {
  ...
  return gulp
    ...
    .on('end', () => {
      ...
      fs.readdir('snippets/', (error, files) => {
        if (error) throw error;
        const mjmlFilesList = files
          .filter((e) => path.extname(e).toLowerCase() === '.mjml')
          .map((file) => `snippets/${file}`);

        async.eachSeries(mjmlFilesList, (filename, cb) => {
